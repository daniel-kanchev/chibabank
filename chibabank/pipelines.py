...
class DatabasePipeline:
    # Database setup
    conn = sqlite3.connect('chibabank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute(""" CREATE TABLE IF NOT EXISTS articles 
        (title text, date text, link text) """)
    ...
    def process_item(self, item, spider):
        ...
        # Insert values
        self.c.execute("INSERT INTO articles (title, date, link)"
                       " VALUES (?,?,?)", (item.get('title'), item.get('date'), item.get('link')))
